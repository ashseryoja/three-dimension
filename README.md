...
Кресло построено процедурно в `scene.js` по референсу заказчика: каркас — «протяжка» круглого сечения
вдоль кривых (функция `sweep`: гнутые ножки, подлокотники, царги), подушки — скруглённые боксы с выпуклостью
граней (`cushion`) и UV-проекцией в мировом масштабе, чтобы тайлящиеся ткани ложились без искажений.

Материалы — настоящие PBR-наборы с Poly Haven (CC0, см. `assets/tex/LICENSE.txt`): букле, тедди, шерсть,
велюр, лён, кожа, дуб, орех. Узор на букле (`assets/tex/boucle_pattern.webp`) сгенерирован скриптом.
Стартовый набор весит ~0,6 МБ, остальные подгружаются в фоне после первого кадра, поэтому в конфигураторе
